...
class BaseConnection(object):
    ...
    async def _lock_netmiko_session(self, start=None):
    ...
    async def _read_channel_expect(self, pattern="", re_flags=0, max_loops=150):
        ...
        while i < max_loops:
            ...
            if re.search(pattern, output, flags=re_flags):
                ...
                return output
            await asyncio.sleep(loop_delay * self.global_delay_factor)
            i += 1
        raise NetmikoTimeoutException(
            f"Timed-out reading channel, pattern not found in output: {pattern}"
        )

    async def _read_channel_timing(self, delay_factor=1, max_loops=150):
        ...
        i = 0
        while i <= max_loops:
            await asyncio.sleep(loop_delay * delay_factor)
            new_data = self.read_channel()
            if new_data:
                channel_data += new_data
            else:
                # Safeguard to make sure really done
                await asyncio.sleep(final_delay * delay_factor)
                new_data = self.read_channel()
                if not new_data:
                    break
                else:
...
"""
    ...
    async def _test_channel_read(self, count=40, pattern="") -> object:
        ...
        async def _increment_delay(main_delay, increment=1.1, maximum=8):
        ...
        await asyncio.sleep(main_delay * 10)
        new_data = ""
        while i <= count:
            ...
            main_delay = await asyncio.create_task(_increment_delay(main_delay))
    ...
    def set_base_prompt(
        ...
        :type delay_factor: int
        """
        prompt = await asyncio.create_task(self.find_prompt(delay_factor=delay_factor))
    ...
    async def clear_buffer(self, backoff=True):
    ...
    def send_command_timing(
        ...
        if cmd and cmd_verify:
            # Make sure you read until you detect the command echo (avoid getting out of sync)
            new_data = await asyncio.create_task(self.read_until_pattern(pattern=re.escape(cmd)))
    ...
    async def send_config_set(
        ...
        if self.fast_cli and self._legacy_mode:
            ...
            for cmd in config_commands:
                self.write_channel(self.normalize_cmd(cmd))
                await asyncio.sleep(delay_factor * 0.05)
            # Gather output
            output += self._read_channel_timing(
                delay_factor=delay_factor, max_loops=max_loops
            )
            ...
            for cmd in config_commands:
                ...
                new_output = await asyncio.create_task(self.read_until_pattern(pattern=re.escape(cmd.strip())))
                output += new_output
