...
class CiscoWlcSSH(BaseConnection):
    ...
    async def special_login_handler(self, delay_factor=1):
        ...
        i = 0
        await asyncio.sleep(delay_factor * 0.5)
        output = ""
        while i <= 12:
            ...
            if output:
                if "login as" in output or "User:" in output:
                    ...
                    break
                await asyncio.sleep(delay_factor * 1)
            else:
                self.write_channel(self.RETURN)
                await asyncio.sleep(delay_factor * 1.5)
            i += 1

    async def send_command_w_enter(self, *args, **kwargs):
        """
        ...
        if "Press any key" in output or "Press Enter to" in output:
            ...
            if "802.11b Advanced Configuration" in output:
                ...
                await asyncio.sleep(kwargs["delay_factor"] * 30)
                not_done = True
                i = 1
                while not_done and i <= 150:
                    await asyncio.sleep(kwargs["delay_factor"] * 3)
                    i += 1
                    new_data = ""
